...
struct enactor_t : gunrock::enactor_t<problem_t> {
  ...
  void loop(gcuda::multi_context_t& context) override {
    ...
    column_indices.resize(estimated_nzs, std::numeric_limits<edge_t>::max());
    ...
    auto real_nonzeros = [=] __host__ __device__(vertex_t const& row) -> void {
      ...
      for (auto nz = row_off[row]; nz < row_off[row + 1]; ++nz) {
        ...
        if (col_ind[nz] == std::numeric_limits<edge_t>::max())
          overestimated_nzs += 1;
      }
    ...
    auto itc = thrust::copy_if(
        ...
        [] __host__ __device__(const vertex_t& x) -> bool { return x != std::numeric_limits<edge_t>::max(); });
